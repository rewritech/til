...
- 안티패턴 인식 방법
  - 속성의 최대 개수가 제한되어 있는 경우
  - SQL에서 여러 칼럼을 한번에 어떻게 검색하지?
...
- 목표: 확장 적응성 지원
  - 데이터양이 증가하면 DB 쿼리 성능 저하
  - DB 쿼리 성능 향상과 데이터양이 증가하는 테이블을 지원

- 안티패턴: 테이블 또는 칼럼 복제
  - 트리블: TV드라마 스타트렉의 애완동물로 엄청나게 번식하는 문제가 있음
  - 적은 행의 테이블 조회가 신속해, 테이블 행이 적어야 한다는 그릇된 생각으로 두 가지 안티패턴 발생
    1. 많은 행을 가진 테이블을 여러 개의 작은 테이블로 분리한다
    1. 하나의 칼럼을 여러 개의 칼럼으로 분리한다
    - 지나치게 많은 칼럼 및 테이블 생성
    - 테이블이 우글우글
      - 어떤 행을 어떤 테이블로 보낼지 결정 정책 필요
      - 연도별 테이블 관리는 새해에 해당 테이블 없으면 에러 발생
    - 데이터 정합성 관리
      - 해당 테이블에 안 맞는 데이터 입력 제한 필요(CHECK제약)
    - 데이터 동기화
      - 데이터 UPDATE 처리가 복잡해짐
      - 기존 테이블에서 삭제후 다른 테이블에 등록
    - 유일성 보장
      - PK칼럼은 다른 테이블 데이터에서도 중복되지 않게 처리 필요
      - 시퀀스 객체가 없는 DB는 PK생성관리용 테이블 생성 필요
    - 여러 테이블에 걸쳐 조회하기
      - 집계기능은 모든 테이블을 UNION하여 구현
      - 새로운 테이블 추가시, 기존 집계기능에 UNION으로 새 테이블 추가
    - 메타데이터 동기화
      - 새 칼럼 추가 시, 모든 테이블에 추가작업 필요
    - 참조 정합성 관리
      - 부모 테이블이 분할된 경우, 참조용 부모 테이블로 지정될 수 없음
      - 자식 테이블이 분할된 경우, 집계 위해 모두 UION으로 연결 필요
    - 메타데이터 트리블 칼럼 식별하기
      - 증가하는 칼럼으로 DB변경 발생
- 안티패턴 인식 방법
  - ~당 테이블 또는 칼럼 생성해야 해
  - 이 DB에 테이블을 퇴개 몇 개까지 만들 수 있을까
    - 대부분 DB는 정말 많은 테이블과 칼럼 다룰 수 있음
    - 그럼에도 최대치를 초과할 것 같다면, 설계 재고 필요
  - 새 해에 대한 테이블 생성을 깜빡해 새 해 데이터 입력이 실패했다
    - 새 데이터에 필요한 새 DB객체는 사전에 정의 필요
  - 같은 칼럼을 지닌 여러 테이블을 어떻게 하면 일괄검색이 가능할까
    - 행 구분용 칼럼과 별토의 태아블 저장했어야 함
  - 어떻게 하면 데이플 이름을 파라미터로 넘길 수 있을까
    - 데이터가 한 테이블에 있다면 불필요

- 안티패턴 사용이 합당한 경우
  - 사용도가 낮은 오래된 데이터는 분리해 별도로 저장
  - 가끔 필요한 대량 데이터는 동일한 테이블 구조로 별도 보관

- 해법: 파티션과 정규화
  - 테이블 분리 보다 성능을 향상할 수 있는 방법 존재
  - 수평 분할(horizontal partitioning) 사용
    - 행을 여러 파티션으로 분리하는 규칙과 함께 논리적 테이블 정의
    - 물리적으로 테이블이 분리되어 있지만, SQL은 한 테이블 처럼 사용 가능
    - 예) MySQL 5.1에서 지원하는 파티셔닝 사용
      - date_reported 칼럼 연도별 분리
      - 파티셔닝은 SQL표준은 아니지만 여러 DB에서 제공
      ...
      PARTITIONS 4;  -- 행 저장용 별도 물리적 테이블 개수
      ```

  - 수직 분할(vertical partitioning) 사용
    - 칼럼으로 테이블을 나눔
    - 크기가 큰 칼럼이나 거의 안 사용하는 칼럼 분리
      - BLOB, TEXT 칼럼은 조회에서 제외하면 성능에 유리
      - \* 와일드카드 사용이 빈번하다면 칼럼을 분리
    - 기준 테이블 PK이 FK인 종속 테이블 생성
    - MySQL 경우 가변 길이 데이터 타입(Varchar)이 테이블에 하나라도 있으면 성능이 안 좋음
      - 모든 가변 길이 타입 칼럼이 조회빈도가 낮다면 별도 테이블에 저장

  - 종속 테이블 사용(메타데이터 트리블 칼럼 고치기)
    - 프로젝트 데이터 행에 연도별 칼럼(year2010, year2020)을 사용하지 말고
    - 연도 칼럼(year)을 만들고 데이터를 연도별로 여러 행으로 저장
      - 새로운 연도 칼럼 추가 불필요
