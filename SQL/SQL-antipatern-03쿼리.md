...
- 안티패턴 인식 방법
  ...
  - 괴물 같은 SQL야
    - 너무 복잡한 SQL
  - 쿼리에 DISTINCT 추가해 볼까?
    - 카테시안 곱 제거하지만 DB 부가작업 발생
  - 쿼리 실행 시간이 지나치게 긴 경우

- 안티패턴 사용이 합당한 경우
  - 프레임워크나 비주얼 컴포넌트 라이브러리 사용
  - 간단한 BI도구 및 리포팅 도구 사용
  - 리포트 요구사항이 복잡해 한 SQL로 만들기 어렵다면, 리포트를 나누는 것이 나음
    - 상사를 설득하자
  - 여러 쿼리 결과 정렬은 애플리케이션 보다 DB에서 수행하는 것이 효율적인 편

- 해법: 분할해서 정복하기
  > 검약률: 두 개의 이론이 동일한 예측을 한다면, 단순한 쪽이 좋은 이론이다.
  - 한 번에 하나씩
    - 조인 조건이 없다면, 쿼리를 나누자
    - 장점
      - 카테시안 곱 방지로 정확한 결과에 대한 확신 가능
      - 새로운 요구사항 추가가 수월
      - SQL엔진은 단순한 쿼리를 쉽게 최적화하는 이득
      - 코드 검포 및 팀원 훈련에 복잡한 한 쿼리 설명 보다 간단한 여러 쿼리 설명이 쉬움

  - UION 연산
    - UNION연산으로 여러 쿼리 결과를 하나의 결과 집합으로 묶음
    - 서브쿼리 칼럼이 호환될 때 UION연산 가능
    - 각 서브쿼리를 구분할 수 있는 칼럼 추가(예: status)
  ...
  - 상사의 문제 해결하기
    - 작업 분리 수행(아래는 예)
      ...
      - 개발자 당 편균 수정 버그 수
      - 수정한 버그 중 고객이 보고한 수
    - 위의 예 중 하나만으로도 충분히 까다로운 SQL 존재
      - 통합한다면 너무 어려움

  - SQL을 이용한 SQL 자동 생성
    - 조금씩 변하는 비슷한 쿼리 반복 생성하는 경우 코드 생성 적용
      - 코드 생성(code generation): 새 코드 출력하는 코드 작성
      - 쿼리를 하나씩 수행하는 SQL문 집합 스크립트
...
- 목표: 타이핑 줄이기
- 안티패턴: 지름길만 좋아하면 길을 읽는다
  - 와일드카드는 타이핑을 줄이지만 위험을 초래
  - 리팩터링 방해
    - INSERT
      - 칼럼 지정이 없으므로 칼럼추가 후 데이터 등록 에러
      - 칼럼명이 없으면 엉뚱한 칼럼에 값 할당 위험
    - SELECT
      - 칼럼명이 없어 칼럼 순서 데이터 참조
      - 중간 칼럼 삭제시 애플리케이션은 엉뚱한 칼럼 참조
    - 애플리케이션의 에러 추적이 어려움

  - 숨겨진 비용
    - 성능과 확장적응성에 안 좋음
      - DB와 애플리케이션 사이 더 많은 데이터 전달
      - 사용사 수에 비례해서 더 많은 쿼리실행으로 기가바이트 네트워크도 포화

  - 요청한 것을 얻은 것이다
    - 지정한 몇몇 칼럼을 제외한 모든 칼럼 요청은 없다.
    - 필요한 칼럼을 모두 명시적으로 나열하자

- 안티패턴 인식 방법
  - DB 수정하고 모든 코드를 수정했지만, 어딘가 빠뜨린 거 같아
  - 네트워크 병목 추적 결과 실제 사용 데이터에 비해 쿼리가 많은 데이터를 가져오고 있어

- 안티패턴 사용이 합당한 경우
  - 임시 사용 SQL
  - 실제 사용 환경 애플리케이션 코드에 SQL와일드카드 거의 사용하지 않음
  - 조인한 테이블의 칼럼 목록이 많은 경우
    ```sql
    SELECT b.*, a.first_name, a.email
    FROM Bugs b JOIN Accounts a
    ON (b.reported_by = a.account_id);
    ```
  - 그러나, 쿼리 자동생성성을 응용해 칼럼 목록 작성 가능
    ```sql
    SELECT column_name || ','
...
- 해법: 명시적으로 칼럼 이름 지정하기
  - 아래의 이유로 항상 필요한 칼럼을 나열해야 함
  - 오류 검증(빨리 실패하는 원칙)
    - 테이블의 칼럼 위치 바뀌어도, 쿼리 결과는 바뀌지 않음
    - 테이블에 칼럼이 추가되어도, 쿼리 결과에는 나타나지 않음
    - 테이블의 칼럼이 삭제되면, 쿼리가 에러를 발생시켜 고칠 코드 위치 파악 가능
    - INSERT 문 칼럼명 지정
      - 테이블 정의 칼럼 순서와 상관없이 칼럼명 대로 추가
      - 지정하지 않은 칼럼은 디폴트 값이나 NULL 등록
      - 삭제된 칼럼을 참조하는 경우, 에러 발생으로 빠른 문제 해결 가능

  - 그거 필요하지 않을 거야
    - SQL와일드카드 포기로 초당 수천 개 SQL 실행 대역폭 효율화 가능
